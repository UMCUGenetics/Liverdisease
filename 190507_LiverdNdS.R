...
for(i in 1:nrow(alc.hcc.all.ns)) {
  ...
  if(nrow(temp.df) == 1) {
    ...
    temp.df$sample <- NA
    if (length(alc.dnds$annotmuts[which(alc.dnds$annotmuts[,7] == mut_gene),1]) == 1 ) {
      temp.df$sample <- alc.dnds$annotmuts[which(alc.dnds$annotmuts[,7] == mut_gene),]$sampleID
    }
    if (length(alc.dnds$annotmuts[which(alc.dnds$annotmuts[,7] == mut_gene),1]) > 1 ) {
      temp.df$sample <- paste(alc.dnds$annotmuts[which(alc.dnds$annotmuts[,7] == mut_gene),]$sampleID, collapse = ",")
    }
...
for(i in 1:nrow(healthy.hcc.all.ns)) {
  ...
  if(nrow(temp.df) == 1) {
    ...
    temp.df$sample <- NA
    if (length(healthy.dnds$annotmuts[which(healthy.dnds$annotmuts[,7] == mut_gene),1]) == 1 ) {
      temp.df$sample <- healthy.dnds$annotmuts[which(healthy.dnds$annotmuts[,7] == mut_gene),]$sampleID
    }
    if (length(healthy.dnds$annotmuts[which(healthy.dnds$annotmuts[,7] == mut_gene),1]) > 1 ) {
      temp.df$sample <- paste(healthy.dnds$annotmuts[which(healthy.dnds$annotmuts[,7] == mut_gene),]$sampleID, collapse = ",")
    }
    cosmic.genes.mutations <- rbind(cosmic.genes.mutations,temp.df)
  }
  remove(mut_gene,temp.df)
